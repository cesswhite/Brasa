...
test('new color rewards unlock at their own milestones and cannot change combat power',async()=>{
 ...
 try {
  ...
  const colors=catalog.items.filter(i=>i.slot==='palette_id');
  assert.equal(colors.length,12);
  for(const id of ['cempasuchil','turquesa','grana','cacao','marfil','cobre','amatista']) {
   const item=colors.find(i=>i.id===id),row=await readOnline(f.db,fighter.fighter_id);
   ...
   const denied=await request(f.mf,owner,'/v1/fighters/'+fighter.fighter_id,'PATCH',{expected_revision:fighter.revision,appearance:{palette_id:id}});
   ...
   const equipped=await request(f.mf,owner,'/v1/fighters/'+fighter.fighter_id,'PATCH',{expected_revision:fighter.revision,appearance:{palette_id:id}});
