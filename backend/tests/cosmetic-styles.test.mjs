...
test('new style rewards require authoritative ownership and preserve combat power',async()=>{
 ...
 try {
  ...
  const effects=catalog.items.filter(i=>['intro_animation_id','victory_pose_id'].includes(i.slot));
  assert.equal(effects.length,8);
  for(const id of ['reverencia','bruma','serena','festival']) {
