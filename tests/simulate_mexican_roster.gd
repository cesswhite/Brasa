...
func _init()->void:
 ...
 var runs:int=0 if OS.get_cmdline_user_args().has("--league-only") else (1 if quick else 3)
 var output:String="res://reports/mexican_roster_balance.json"
  ...
  for level:int in [1,10,25,50]:
   ...
   for foe_index:int in range(Catalog.IDS.size()):
    ...
    for sample:int in range(samples):
     var seed_value:int=87100000+hero_index*1000000+level*10000+foe_index*100+sample
   ...
   print("MEXICAN league %s L%d %.1f%%" % [id,level,100.0*wins/played])
 ...
 for hero_index:int in range(IDS.size()):
  ...
  for build_index:int in range(Audit.BUILDS.size()):
   ...
   for repetition:int in range(runs):
    ...
    p.data.run_seed=16710000+hero_index*100000+build_index*1000+repetition
    var cursor:int=0
    var fights:int=0
    ...
    while not p.campaign_completion().completed and fights<500:
     ...
     while not won and attempts<Config.MAX_STAGE_ATTEMPTS:
      ...
      var result:Dictionary=_fight(p.active_combatant(),p.make_rival(),983000000+hero_index*1000000+build_index*100000+repetition*10000+fights)
   ...
   print("MEXICAN story %s/%s %d/%d complete %.1f battles" % [id,build,done,runs,float(fights)/maxi(1,runs)])
